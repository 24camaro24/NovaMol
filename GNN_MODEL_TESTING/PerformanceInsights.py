...
def get_chembl_data(target_id='CHEMBL203', min_pchembl=5.0):
    ...
    raw_csv_path = os.path.join(os.getcwd(), f"{target_id}_raw_chembl.csv")
    ...
    processed_csv_path = os.path.join(os.getcwd(), f"{target_id}_processed_analysis.csv")
...
def run_complete_pipeline(target_id: str, output_dir: str = 'results', progress_file: str = None, gnn_class = MultiTaskGNN_v6):
    ...
    gnn_model = gnn_class(in_dim=train_data[0].num_node_features, out_dim=N_PROPERTIES).to(DEVICE)
    ...
    metrics_df.to_csv(os.path.join(os.getcwd(), f"{target_id}_{gnn_model.__class__.__name__}_val_metrics_history.csv"), index=False)
    ...
    test_metrics_df.to_csv(os.path.join(os.getcwd(), f"{target_id}_{gnn_model.__class__.__name__}_test_metrics.csv"), index=False)

    return test_maes_report, proof_df


def run_all_gnn(dataset = 'CHEMBL203'):
    gnn_classes = [
        MultiTaskGNN_v1,
        MultiTaskGNN_v2,
        MultiTaskGNN_v3,
    ...
    for gnn_cls in gnn_classes:
        ...
        run_complete_pipeline(dataset, gnn_class=gnn_cls)


# To read and visualize the results:
    df_gnn_v1 = pd.read_csv(f'{dataset}_MultiTaskGNN_v1_val_metrics_history.csv')
    df_gnn_v2 = pd.read_csv(f'{dataset}_MultiTaskGNN_v2_val_metrics_history.csv')
    df_gnn_v3 = pd.read_csv(f'{dataset}_MultiTaskGNN_v3_val_metrics_history.csv')
    df_gnn_v4 = pd.read_csv(f'{dataset}_MultiTaskGNN_v4_val_metrics_history.csv')
    df_gnn_v5 = pd.read_csv(f'{dataset}_MultiTaskGNN_v5_val_metrics_history.csv')
    df_gnn_v6 = pd.read_csv(f'{dataset}_MultiTaskGNN_v6_val_metrics_history.csv')
    ...
    plt.tight_layout()
    # plt.savefig('gnn_comparison_metrics.png')
    plt.show()

    files = [
        f"{dataset}_MultiTaskGNN_v1_test_metrics.csv",
        f"{dataset}_MultiTaskGNN_v2_test_metrics.csv",
        f"{dataset}_MultiTaskGNN_v3_test_metrics.csv",
        f"{dataset}_MultiTaskGNN_v4_test_metrics.csv",
        f"{dataset}_MultiTaskGNN_v5_test_metrics.csv",
        f"{dataset}_MultiTaskGNN_v6_test_metrics.csv"
    ]
    ...
    plt.style.use('seaborn-v0_8-whitegrid') # Set a nice plotting style
    plt.subplots_adjust(hspace=0.45) # Increase vertical spacing

    # Consistent model order for the hue legend
    model_order = [f'v{j}' for j in range(1, 7)]

    for i, prop in enumerate(properties):
        df_prop = df_long[df_long['Property'] == prop]

        # Filter to only the 4 main metrics
        ...
        sns.barplot(
            ...
            palette='magma',
            ax=axes[i],
            order=['MAE', 'MSE', 'RMSE', 'R2'], # Fixed metric order
            hue_order=model_order,
            errorbar=None # Single-row data means no error bar needed
        )

        # Set titles and labels
        # Use the clean label for the title
        property_label = df_prop_filtered['Property_Label'].iloc[0]
        axes[i].set_title(f'Model Performance Comparison for: {property_label}', fontsize=16, fontweight='bold')
        axes[i].set_xlabel('Metric Type', fontsize=14)
        axes[i].set_ylabel(f'Metric Value for {property_label}', fontsize=14)
        axes[i].legend(title='Model Version', loc='upper right')

        # Add annotation for goal (Crucial for interpretation!)
        # Use a raw string (r'...') to avoid the SyntaxError with LaTeX escapes
        axes[i].text(
            0.98,
            0.98,
    ...
    df_heatmap = df_heatmap[metric_columns_order]
